...
dhtmlXForm.prototype.items.upload = {
	...
	render: function(item, data) {
		...
		item._uploader = new dhtmlXFileUploader(k, data.swfPath||"", data.swfUrl||"", data.mode||null, data.swfLogs, data.slXap||"", data.slUrl||"", data.slLogs||"");
...
function dhtmlXFileUploader(p, swfPath, swfUrl, mode, swfLogs, slXap, slUrl, slLogs) {
...
dhtmlXFileUploader.prototype.html5.prototype = {
	...
	_initEngine: function() {
		...
		// "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/534.57.2 (KHTML, like Gecko) Version/5.1.7 Safari/534.57.2"			// ok		
	...
	_addFileInput: function() {
		...
		this.f.onchange = function() {
			...
			if (_isOpera) that._addFileInput();
		}
	},
		...
		if (!this._loader) {
			...
			this._loader.onload = function(e) {
				...
				if (typeof(r) == "object" && typeof(r.state) != "undefined" && r.state == true) {
					that._onUploadSuccess(this.upload._idd, r.name);
					r = null;
				} else {
...
dhtmlXFileUploader.prototype.flash.prototype = {
	...
	_addFileToQueue: function(id, name, size) {
		if (_isIE) {
			...
			k.style.left = "0px";
			k.style.top = getAbsoluteTop(this.buttons["browse"])+"px";
			k.style.width = "10px";
...
dhtmlXFileUploader.prototype.sl.prototype = {
	...
	_initEngine: function() {
		...
		this._sl_obj_id = "dhtmlXFileUploaderSLObject_"+window.dhtmlXFileUploaderSLObjects.uid();
	...
	_doUploadFile: function(id) {
		document.getElementById(this._sl_obj_id).Content.a.upload(id, this._sl_upload_url, "&mode=sl&etc="+new Date().getTime()); // leading "&" required!
	},
